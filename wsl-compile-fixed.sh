...
echo ""
echo "步骤 2.5: 修复脚本换行符..."
find . -type f \( -name "*.sh" -o -name "autogen.sh" -o -name "configure.ac" \) -exec dos2unix {} \; 2>/dev/null || {
    echo "安装 dos2unix..."
    sudo apt-get update -qq
    sudo apt-get install -y dos2unix
    find . -type f \( -name "*.sh" -o -name "autogen.sh" -o -name "configure.ac" \) -exec dos2unix {} \;
}
echo "✓ 脚本换行符已修复"
...
if [ ! -f "/usr/local/lib/libgmssl.so.3.1" ]; then
    echo "❌ 错误: GmSSL 未安装,请先运行 GmSSL 安装"
    exit 1
fi
...
if [ ! -f "$PLUGIN_PATH" ]; then
    echo "❌ 错误: 插件未生成"
    echo "查看可能的位置:"
    find . -name "libstrongswan-gmsm.so" -o -name "*gmsm*.so" 2>/dev/null
    exit 1
fi
